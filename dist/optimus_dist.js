...
var bower_components_requirejs_require, DOMCompiler, scope, directives_controller, directives_model, directives_bind, directives_click, directives_show, directives_directivesLoader, OPTIMUS;
...
bower_components_requirejs_require = undefined;
DOMCompiler = function () {
  var compiler = {
    provider: {},
    ...
    compile: function (el, scope) {
      var that = this, dirs = that._getElDirectives(el), dir, scopeCreated;
    ...
    _getElDirectives: function (el) {
      var that = this, attrs = el.attributes, result = [];
...
directives_controller = function () {
  var controller = {
    scope: true,
...
directives_show = function () {
  var show = {
    scope: false,
    link: function (el, scope, exp, provider) {
      var CLASSNAME = 'pt-hidden', newProp = scope.PRIVATE_PREFIX + exp, value = scope.eval(exp);
      scope[newProp] = value;
      if (value) {
...
directives_directivesLoader = function (require) {
  var controller = directives_controller, model = directives_model, bind = directives_bind, click = directives_click, show = directives_show;
  var loader = {
    directives: {
...
OPTIMUS = function (require) {
  var compiler = DOMCompiler, Scope = scope, directivesLoader = directives_directivesLoader;
  var Provider = {
    ...
    annotate: function (fn) {
      var res = fn.toString().replace(/((\/\/.*$)|(\/\*[\s\S]*?\*\/))/gm, '').match(/\((.*?)\)/);
      if (res && res[1]) {
    ...
    bootstrap: function () {
      ...
      // start the engine
      compiler.bootstrap(this);
    }
  };
  return Provider;
}({});
window.OPTIMUS = OPTIMUS;
}());